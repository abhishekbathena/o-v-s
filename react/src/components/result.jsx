        ...
        <div>
          ...
          <div style={{ paddingRight: "10px;" }} class="container">
            ...
            <div class="row" style={{marginTop:'150px'}}>
              {cand.map((value, key) => {
                axios
                  .post(`http://localhost:8000/reg/winner/${value.id}`)
                  .then((res) => {
                    if (res.data.s === "fail") {
                      console.log(res);
                      setbt(true);
                  ...
                  <div class="col-sm-12 col-md-4">
                    ...
                    <div style={{ textAlign: "center", color: "black" }}>
                      <a
                        ...
                        onClick={() => {
                          axios
                            .get(`http://localhost:8000/reg/count/${value.id}`)
                            .then((res) => {
                      ...
                      <h1 style={{color:'black'}}>{candi == value.id ? count : ""}</h1>
                    </div>
                  </div>
                );
              })}
...
function Cardres(props) {
  ...
  const [listofposition, setlistofposition] = useState([]);
  useEffect(() => {
    axios.get("http://localhost:8000/reg/positioni").then((res) => {
      setlistofposition(res.data);
      console.log(res.data);
    });
  }, []);
